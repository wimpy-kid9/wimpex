...
export default function BottomNav() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 md:hidden">
      <nav className="relative mx-auto flex max-w-3xl items-center justify-between gap-2 border-t border-white/10 bg-slate-950/90 p-3 text-sm text-slate-200 backdrop-blur-xl">
        <div className="absolute left-0 right-0 top-0 flex h-full items-center justify-start">
          <div className="mx-auto w-full max-w-3xl relative">
            <div
              className="pointer-events-none absolute left-0 top-1/2 h-9 w-[20%] -translate-y-1/2 transform rounded-lg bg-amber-400/15 transition-all duration-300"
        ...
        {items.map((item) => (
          <Link key={item.href} href={item.href} className={`relative z-10 flex w-full flex-col items-center gap-1 rounded-2xl px-3 py-2 transition ${isActive(item.href) ? 'text-white' : 'hover:text-white'}`}>
