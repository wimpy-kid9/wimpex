...
export default function BottomNav() {
  ...
  useEffect(() => {
    const updateCount = () => setTotalUnread(getTotalUnreadCount());
    updateCount();

    const handler = () => updateCount();
    window.addEventListener('wimpex-chat-unread-updated', handler);
    return () => window.removeEventListener('wimpex-chat-unread-updated', handler);
  }, []);
            ...
            <Link key={item.href} href={item.href} className={`relative z-10 flex w-full flex-col items-center gap-1 rounded-2xl px-3 py-2 transition ${isActive(item.href) ? 'text-ivory' : 'hover:text-ivory'}`}>
              <span className="relative h-5 w-5">
                ...
                {isChatItem && totalUnread > 0 ? (
                  <span className="absolute -right-2 -top-2 flex min-h-4 min-w-4 items-center justify-center rounded-full bg-rose-500 px-1 text-[9px] font-bold leading-none text-white shadow-sm">
